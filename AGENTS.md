...
浏览器、原生客户端及其他端到端动态测试，必须在本任务获准范围内的全部静态门禁通过后
...
画面静止/测试卡死并退出。整个任务的测试流程从首条测试命令开始共享 60 分钟墙钟预算；
超时立即停止，并报告命令、用例/阶段、最后完整快照、已用时间及未验证项。
...
每个触发上述条件的任务，重构子智能体必须且只能运行一次。它必须在任何测试启动前完成
对全部任务代码的完整审查；主智能体必须在首条测试命令前解决其提出的所有重构要求。不得在
测试开始后新建、恢复、追加轮次或再次启动该子智能体；也不得以二次审查替代主智能体对首次
审查要求的完整落实。
...
- 先运行与改动直接相关的最小 Vitest，再按顺序运行一次完整前端门禁：
