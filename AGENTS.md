...
每条测试命令必须委派给运行 **gpt-5.6-terra low** 的子智能体。主智能体必须先提供完整、
可直接执行的命令及执行说明，明确工作目录、工具路径、环境变量、全部参数、输入输出、
执行顺序、超时/看门狗/截止时间、结果收集和停止清理步骤；不得留占位符或要求其自行补全。
测试子智能体严禁猜测任何内容，仅能原样执行给定命令、按指令监督并收集结果；不得自行
选择测试、推断参数或路径、诊断修复、重建重试、修改超时或判断验收。缺项、歧义或异常
必须返回主智能体，等待新的完整命令；仅继续已明确规定的监督、停止和清理步骤。
唯一例外：用户明确要求自主游玩且主智能体确认该阶段门槛已满足时，子智能体可自行选择
已授权范围内的游戏内行动；启动、操作接口、边界、监控与退出命令仍由主智能体完整提供，
不得据此自行决定任何测试流程、环境、参数、重试或故障处理。该子智能体只能执行测试
...
测试、fixture、依赖或构建输入发生变化，必须立即告知测试子智能体；由主智能体确定受影响
检查及必要重建，提供新的完整命令后再执行；受影响的旧结果一律作废。
