...
import { runSnakeServicesClient } from "../../scripts/snake-services-test-support.mjs";
const enabled = process.env.VITE_RUSTYERA_TAURI_SNAKE_SERVICES === "1" ? describe : describe.skip;
enabled("Tauri snake services integration", () => {
  it("executes core-issued services using the real host and visible controls", async () => {
    ...
    const observed = await runSnakeServicesClient(browser, "tauri");
    console.log(
      JSON.stringify({
